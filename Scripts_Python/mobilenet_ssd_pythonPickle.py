...
for idx, img in enumerate(boxes):
    save_path = os.path.join(save_directory, f'image_with_box_{idx}.png')
    if img is not None and img.size != 0:
        cv2.imwrite(save_path, img)
    else:
        print("image vide non prise en charge")
...
for filename in os.listdir(save_directory):
    if filename.endswith(".png"): 
        ...
        if img is not None:
            ...
            print(f"Unable to read picture : {filename}")

pickle_file = "images_data.pickle"
